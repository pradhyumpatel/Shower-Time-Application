...
sed -i "s/\$ACCESSFROMIP/127.0.0.1/g" ~/2021-team01t.git/sprint-02/code/db-samples/*.sql
sed -i "s/\$USERPASS/$USERPASS/g" ~/2021-team01t.git/sprint-02/code/db-samples/*.sql
sed -i "s/\$MMIP/127.0.0.1/g" ~/2021-team01t.git/sprint-02/code/db-samples/*.sql
# This script will create the database named posts in the mariadb server
sudo mysql -u root < ~/2021-team01t.git/sprint-02/code/db-samples/create-database.sql
# This script will create the table named comments
sudo mysql -u root < ~/2021-team01t.git/sprint-02/code/db-samples/create-table.sql
# This script will create the non-root user named worker and the user for replication
sudo mysql -u root < ~/2021-team01t.git/sprint-02/code/db-samples/create-user-with-permissions-mm.sql
# This script will insert 3 sample records to the table
sudo mysql -u root < ~/2021-team01t.git/sprint-02/code/db-samples/insert-records.sql
# This script will select * from comments and print the contents to the screen to make sure it all worked
sudo mysql -u root < ~/2021-team01t.git/sprint-02/code/db-samples/sample-select.sql
